...
from vis_keras import vis_core as vc
from vis_keras import vis_utils as vu
...
class Model_explorer():
    ...
    def set_test_image(self, img_path):
        if len(self.input_shape) is 4:
            ...
            self.img = vu.io.load_2d(img_path, t_size)
            self.tensor = vu.io.to_tensor(self.img)
    ...
    def cam(self):
        vc.grad_cam(self.model, self.tensor)
